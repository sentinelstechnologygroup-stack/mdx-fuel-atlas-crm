...
Deno.serve(async (req) => {
  try {
    ...
    const roleDefs = await atlasRuntime.asServiceRole.entities.RoleDefinition.list(200);
    ...
    if (actorRole !== 'super_admin') {
      ...
      if (roleDef.role_key === 'super_admin' || roleDef.role_key === 'administrator') {
        await writeAudit(atlasRuntime, user, { event: 'denied_protected_role_edit', role_key, module_key, reason });
      ...
      if (roleDef.role_type === 'custom' && (roleDef.base_role_key === 'super_admin' || roleDef.base_role_key === 'administrator')) {
        await writeAudit(atlasRuntime, user, { event: 'denied_admin_custom_role_edit', role_key, module_key, reason });
      ...
      if (ADMIN_TIER_MODULES.includes(module_key) && fields.can_manage_configuration === true) {
        await writeAudit(atlasRuntime, user, { event: 'denied_admin_config_grant', role_key, module_key, reason });
    ...
    const existing = await atlasRuntime.asServiceRole.entities.ModulePermission.filter({ role_key, module_key, status: 'active' }, 5);
    let record;
    let previous = null;
    if (existing && existing.length > 0) {
      ...
      record = await atlasRuntime.asServiceRole.entities.ModulePermission.update(record.id, {
        ...clean,
        last_modified_by_user_id: user.id,
        modified_date: now
      });
    } else {
      record = await atlasRuntime.asServiceRole.entities.ModulePermission.create({
