...
//
// Rules (Phase 3B.1):
//   1. Only the legacy platform project-owner administrator (role === 'admin') is a
//      bootstrap candidate. Ordinary legacy platform users are never promoted.
...
//      bootstrap another. We never promote every legacy platform administrator.
...
//   6. This works even if legacy platform applied the schema default
...
Deno.serve(async (req) => {
  try {
    ...
    // Only the legacy platform project-owner admin is a bootstrap candidate.
    if (user.role !== 'admin') {
    ...
    if (user.application_role === 'super_admin') {
      ...
      await atlasRuntime.asServiceRole.entities.User.update(user.id, {
