...
Deno.serve(async (req) => {
  try {
    ...
    for (const id of ids) {
      try {
        const record = await atlasRuntime.asServiceRole.entities[entityName].get(id);
        ...
        await atlasRuntime.asServiceRole.entities[entityName].update(id, update);
        ...
        let restored = false;
        try {
          await atlasRuntime.asServiceRole.entities[entityName].update(id, previousState);
          restored = true;
        ...
        if (restored) {
          ...
          try { await atlasRuntime.asServiceRole.entities[entityName].update(id, { ownership_status: 'transfer_pending' }); } catch (_e) {}
