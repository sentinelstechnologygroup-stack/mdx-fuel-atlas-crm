...
Deno.serve(async (req) => {
    try {
        ...
        const leads = await atlasRuntime.entities.Lead.filter({ id: leadId });
        const lead = leads[0];
        ...
        const aiResult = await atlasRuntime.integrations.Core.InvokeLLM({
...
חשוב: אל תמציא מידע שלא מופיע בטקסט!`,
        ...
        if (Object.keys(leadUpdates).length > 0) {
            await atlasRuntime.entities.Lead.update(leadId, leadUpdates);
        }
