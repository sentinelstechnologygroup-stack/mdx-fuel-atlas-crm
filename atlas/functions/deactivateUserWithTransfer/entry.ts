...
async function validateDestination(atlasRuntime, { toOwnerUserId, toTeamId, toSupervisorId, excludeUserId }) {
  const destUser = await atlasRuntime.asServiceRole.entities.User.get(toOwnerUserId).catch(() => null);
  ...
  if (toTeamId) {
    const team = await atlasRuntime.asServiceRole.entities.Team.get(toTeamId).catch(() => null);
    if (!team) return { error: 'Destination team not found.' };
  }

  let supervisorId = toSupervisorId || null;
  if (toSupervisorId) {
    const sup = await atlasRuntime.asServiceRole.entities.User.get(toSupervisorId).catch(() => null);
    ...
    const sup = await atlasRuntime.asServiceRole.entities.User.get(destUser.supervisor_user_id).catch(() => null);
...
Deno.serve(async (req) => {
  try {
    ...
    for (const cfg of ENTITY_OWNER_CONFIG) {
      const items = await atlasRuntime.asServiceRole.entities[cfg.entity].filter({ [cfg.ownerField]: targetId }, '-created_date', 500);
    ...
    if (transferOption !== 'mark_inactive') {
      ...
      const dest = await validateDestination(atlasRuntime, { toOwnerUserId: destinationUserId, toTeamId: null, toSupervisorId: null, excludeUserId: targetId });
    ...
    for (const cfg of ENTITY_OWNER_CONFIG) {
      ...
      for (const record of recordsToTransfer) {
        try {
          ...
          await atlasRuntime.asServiceRole.entities[cfg.entity].update(record.id, update);
          ...
          let restored = false;
          try {
            await atlasRuntime.asServiceRole.entities[cfg.entity].update(record.id, previousState);
            restored = true;
          ...
          if (restored) {
            ...
            try { await atlasRuntime.asServiceRole.entities[cfg.entity].update(record.id, { ownership_status: 'transfer_pending' }); } catch (_e) {}
      ...
      for (const record of ownedByEntity[cfg.type]) {
        ...
        try {
          await atlasRuntime.asServiceRole.entities[cfg.entity].update(record.id, {
    ...
    await atlasRuntime.asServiceRole.entities.User.update(target.id, {
