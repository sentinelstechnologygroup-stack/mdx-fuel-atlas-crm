...
// deleteAuthorizedRecord — thin, named wrapper over the authorizedRecords gateway.
Deno.serve(async (req) => {
  try {
    const atlasRuntime = createClientFromRequest(req);
    let body = {};
    try { body = await req.json(); } catch (_e) { body = {}; }
    const res = await atlasRuntime.functions.invoke('authorizedRecords', { ...body, operation: 'delete' });
