...
Deno.serve(async (req) => {
  try {
    ...
    if (targetUserId && targetUserId !== user.id) {
      ...
      target = await atlasRuntime.asServiceRole.entities.User.get(targetUserId);
    ...
    if (target.custom_role_id) {
      try {
        customRole = await atlasRuntime.asServiceRole.entities.RoleDefinition.get(target.custom_role_id);
    ...
    const rolePerms = await atlasRuntime.asServiceRole.entities.ModulePermission.filter({ role_key: permRoleKey, status: 'active' }, 200);
    ...
    const overrides = await atlasRuntime.asServiceRole.entities.UserPermissionOverride.filter({ user_id: target.id, status: 'active' }, 200);
