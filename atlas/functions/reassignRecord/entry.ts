...
async function validateDestination(atlasRuntime, { toOwnerUserId, toTeamId, toSupervisorId }) {
...
Deno.serve(async (req) => {
  try {
    ...
    const record = await atlasRuntime.asServiceRole.entities[entityName].get(entityId);
    ...
    const dest = await validateDestination(atlasRuntime, { toOwnerUserId, toTeamId, toSupervisorId });
    ...
    await atlasRuntime.asServiceRole.entities[entityName].update(entityId, update);
    ...
    let restored = false;
    try {
      await atlasRuntime.asServiceRole.entities[entityName].update(entityId, previousState);
      restored = true;
    ...
    try { await atlasRuntime.asServiceRole.entities[entityName].update(entityId, { ownership_status: 'transfer_pending' }); } catch (_e) {}
    try {
      await atlasRuntime.asServiceRole.entities.AuditLog.create({
