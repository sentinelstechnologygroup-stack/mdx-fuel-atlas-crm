...
//
// This file is intentionally self-contained (no local imports) because Base44
...
async function audit(base44, actor, action, entity, entityId, outcome, details) {
  try {
...
async function getDirectReports(base44, user) {
  if (!user || !user.id) return [];
  try {
    const reps = await base44.asServiceRole.entities.User.filter(
...
Deno.serve(async (req) => {
  try {
    ...
    let perm = null;
    try {
      const res = await base44.functions.invoke('getEffectivePermissions', { module_key: moduleKey });
    ...
    if (!allowed && HIGH_RISK.includes(op)) {
      await audit(base44, user, `${op}_denied`, mod.entity, body.record_id, 'denied', {
    ...
    const reports = scope === 'team' ? await getDirectReports(base44, user) : [];
    const repo = base44.asServiceRole.entities[mod.entity];
    ...
    if (op === 'list' || op === 'counts' || op === 'export') {
      ...
      if (op === 'export') {
        ...
        await audit(base44, user, 'export', mod.entity, null, 'success', {
    ...
    if (op === 'update' || op === 'assign') {
      ...
      if (!record) return Response.json({ error: 'Not found.' }, { status: 404 });
      if (!recordInScope(record, scope, user, mod, reports)) {
        await audit(base44, user, 'update_denied', mod.entity, body.record_id, 'denied', {
      ...
      if (mayAssign && (data.owner_user_id !== undefined || data.assigned_team_id !== undefined || data.assigned_supervisor_user_id !== undefined)) {
        ...
        await audit(base44, user, 'assign', mod.entity, body.record_id, 'success', {
    ...
    if (op === 'delete') {
      ...
      if (!record) return Response.json({ error: 'Not found.' }, { status: 404 });
      if (!recordInScope(record, scope, user, mod, reports)) {
        await audit(base44, user, 'delete_denied', mod.entity, body.record_id, 'denied', {
          module_key: moduleKey, reason: 'out of scope'
        });
        return Response.json({ error: 'Access denied for this record.' }, { status: 403 });
      }
      await repo.delete(body.record_id);
      await audit(base44, user, 'delete', mod.entity, body.record_id, 'success', { module_key: moduleKey });
