...
Deno.serve(async (req) => {
  try {
    ...
    const roleDef = await base44.asServiceRole.entities.RoleDefinition.get(role_definition_id);
    if (!roleDef) return Response.json({ error: 'Role definition not found.' }, { status: 404 });
    if (roleDef.role_type !== 'custom') {
      await writeAudit(base44, user, { event: 'denied_system_role_modify', role_key: roleDef.role_key, action });
    ...
    if (actorRole !== 'super_admin' && (roleDef.base_role_key === 'super_admin' || roleDef.base_role_key === 'administrator')) {
      await writeAudit(base44, user, { event: 'denied_admin_custom_role', role_key: roleDef.role_key, action });
    ...
    if (action === 'delete') {
      if (activeAffected.length > 0) {
        await writeAudit(base44, user, { event: 'denied_delete_assigned', role_key: roleDef.role_key, active_users: activeAffected.length });
      ...
      await base44.asServiceRole.entities.RoleDefinition.delete(roleDef.id);
      await writeAudit(base44, user, { event: 'delete', role_key: roleDef.role_key, reason: reason || null, entity_id: roleDef.id });
      return Response.json({ status: 'deleted' });
    }

    const newStatus = action === 'activate' ? 'active' : 'inactive';
    const updated = await base44.asServiceRole.entities.RoleDefinition.update(roleDef.id, {
      status: newStatus,
