...
Deno.serve(async (req) => {
  try {
    ...
    if (action === 'confirm') {
      ...
      const record = await base44.asServiceRole.entities[cfg.entity].get(entityId);
      ...
      await base44.asServiceRole.entities[cfg.entity].update(entityId, update);
      ...
      let restored = false;
      try {
        await base44.asServiceRole.entities[cfg.entity].update(entityId, previousState);
        restored = true;
      ...
      try {
        await base44.asServiceRole.entities[cfg.entity].update(entityId, { ownership_status: 'transfer_pending' });
      } catch (_e) { /* best-effort */ }
      try {
        await base44.asServiceRole.entities.AuditLog.create({
    ...
    for (const cfg of ENTITY_CONFIG) {
      let records = [];
      try {
        records = await base44.asServiceRole.entities[cfg.entity].filter({}, '-created_date', 200);
      } catch (_e) { continue; }
