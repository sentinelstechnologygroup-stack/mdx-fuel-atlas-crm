...
Deno.serve(async (req) => {
  try {
    ...
    const existingRoles = await base44.asServiceRole.entities.RoleDefinition.list(100);
    ...
    for (const def of SYSTEM_ROLE_DEFS) {
      ...
      await base44.asServiceRole.entities.RoleDefinition.create({
    ...
    const allPerms = await base44.asServiceRole.entities.ModulePermission.list(500);
    ...
    if (rolesCreated > 0) {
      const fresh = await base44.asServiceRole.entities.RoleDefinition.list(100);
    ...
    for (const roleKey of Object.keys(DEFAULT_MATRIX)) {
      ...
      for (const moduleKey of ACTIVE_MODULE_KEYS) {
        ...
        await base44.asServiceRole.entities.ModulePermission.create({
