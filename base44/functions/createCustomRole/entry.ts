...
Deno.serve(async (req) => {
  try {
    ...
    if (actorRole !== 'super_admin' && (base_role_key === 'super_admin' || base_role_key === 'administrator')) {
      await writeAudit(base44, user, { event: 'denied_admin_custom_role', name, base_role_key });
    ...
    const existing = await base44.asServiceRole.entities.RoleDefinition.list(200);
    ...
    const roleDef = await base44.asServiceRole.entities.RoleDefinition.create({
      role_key,
    ...
    const basePerms = await base44.asServiceRole.entities.ModulePermission.filter({ role_key: base_role_key, status: 'active' }, 200);
    ...
    if (toCreate.length > 0) {
      await base44.asServiceRole.entities.ModulePermission.bulkCreate(toCreate);
    }

    await writeAudit(base44, user, { role_key, name, base_role_key, permissions_cloned: toCreate.length, entity_id: roleDef.id });
