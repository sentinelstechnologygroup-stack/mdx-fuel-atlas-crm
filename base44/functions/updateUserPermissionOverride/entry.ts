...
Deno.serve(async (req) => {
  try {
    ...
    // No self-escalation.
    if (user_id === user.id) {
      await writeAudit(base44, user, { event: 'denied_self_override', user_id, module_key, reason });
    ...
    if (actorRole !== 'super_admin') {
      ...
      if (tRole === 'super_admin' || tRole === 'administrator') {
        await writeAudit(base44, user, { event: 'denied_admin_tier_override', user_id, module_key, reason });
      ...
      if (override_mode === 'replace' && ADMIN_TIER_MODULES.includes(module_key) && fields.can_manage_configuration === true) {
        await writeAudit(base44, user, { event: 'denied_override_config_grant', user_id, module_key, reason });
    ...
    const existing = await base44.asServiceRole.entities.UserPermissionOverride.filter({ user_id, module_key, status: 'active' }, 5);
    ...
    if (action === 'deactivate') {
      ...
      const updated = await base44.asServiceRole.entities.UserPermissionOverride.update(existingRecord.id, {
        status: 'inactive',
        last_modified_by_user_id: user.id,
        modified_date: now
      });
      await writeAudit(base44, user, { event: 'deactivate', user_id, module_key, reason: reason || null, entity_id: existingRecord.id });
    ...
    if (!reason || !String(reason).trim()) {
      await writeAudit(base44, user, { event: 'denied_missing_reason', user_id, module_key });
    ...
    if (existingRecord) {
      ...
      record = await base44.asServiceRole.entities.UserPermissionOverride.update(existingRecord.id, {
      ...
      record = await base44.asServiceRole.entities.UserPermissionOverride.create({
